...
internal class PleiepengerBarnUttakTestClient(private val restTemplate: TestRestTemplate) {
    ...
    internal fun simulerUttaksplanLivetsSluttfase(grunnlag: Uttaksgrunnlag): ResponseEntity<SimuleringLivetsSluttfase> {
        return restTemplate.exchange(UttakplanApi.UttaksplanSimuleringSluttfasePath, HttpMethod.POST, HttpEntity<Any>(grunnlag, headers()), SimuleringLivetsSluttfase::class.java)
    }

    internal fun nedjusterSøkersUttaksgrad(grunnlag: Uttaksgrunnlag): ResponseEntity<Uttaksplan> {
        return restTemplate.exchange(UttakplanApi.UttaksplanNedjusterSøkersUttaksgradPath, HttpMethod.POST, HttpEntity<Any>(grunnlag, headers()), Uttaksplan::class.java)
    }
